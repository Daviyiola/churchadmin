...
type MemberRow = { id: string; first_name: string; last_name: string | null };
...
export async function runMemberGivingReportAsAdmin(
  ...
  if (view === "summary") {
    ...
    for (const e of entries) {
      ...
      sums.set(cid, (sums.get(cid) ?? 0) + dollarsFromCents(e.amount_cents));
    }
