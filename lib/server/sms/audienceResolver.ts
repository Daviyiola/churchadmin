...
import { currentSmsPermissions, permissionKey } from "@/lib/sms/permissions";
import { getSmsPermissionEvents } from "./permissions";
...
export function parseSmsAudience(value: unknown): SmsAudienceCriteria {
  ...
  return {
    message_category: row.message_category === "promotional" ? "promotional" : "informational",
...
export async function getSmsAudienceOptions(orgId: string) {
  ...
  return {
    ...
    forms: forms.map((form) => ({ ...form, fields: fields.filter((field) => field.form_id === form.id) }))
      .filter((form) => form.fields.some((field) => field.field_type === "phone") && form.fields.some((field) => ["yes_no", "single_choice", "multiple_choice"].includes(String(field.field_type)))),
  };
}
...
export async function resolveSmsAudience(orgId: string, raw: unknown, message: string) {
  ...
  const permissions = currentSmsPermissions(await getSmsPermissionEvents(orgId));
  const category = criteria.message_category ?? "informational";
  ...
  const noConsent = new Set<string>();
  ...
  const add = (phoneRaw: unknown, recipient: Omit<Recipient, "phone_e164" | "personalized_character_count" | "personalized_segments">, firstName: string) => {
    ...
    const permission = permissions.get(permissionKey(normalized.e164, category));
    if (permission?.status !== "granted") { noConsent.add(normalized.e164); return; }
    ...
    recipientMap.set(normalized.e164, { ...recipient, consent_basis: "individual_consent", consent_reference_id: permission.id, phone_e164: normalized.e164, personalized_character_count: estimate.characters, personalized_segments: estimate.segments });
  ...
  for (const item of selected.values()) add(item.person.phone, {
    ...
    consent_basis: "individual_consent", consent_reference_id: null,
  ...
  for (const source of criteria.form_sources) {
    ...
    const { data: consentField } = await supabaseAdmin.from("form_fields").select("field_key,field_type,label").eq("org_id", orgId).eq("form_id", form.id).eq("field_key", source.consent_field_key).in("field_type", ["yes_no", "single_choice", "multiple_choice"]).maybeSingle();
  ...
  return {
    ...
    no_consent_count: noConsent.size,
...
export async function getSmsReadiness(orgId: string) {
  ...
  const counts = { valid: 0, invalid: 0, missing: 0, suppressed: 0, informational: 0, promotional: 0, no_permission: 0 };
  ...
  for (const person of people) {
    ...
    counts.valid += 1;
    if (blocked.has(parsed.e164)) { counts.suppressed += 1; continue; }
    const info = permissions.get(permissionKey(parsed.e164, "informational"))?.status === "granted";
    const promo = permissions.get(permissionKey(parsed.e164, "promotional"))?.status === "granted";
    if (info) counts.informational += 1;
    if (promo) counts.promotional += 1;
    if (!info && !promo) counts.no_permission += 1;
  }
  return counts;
}
