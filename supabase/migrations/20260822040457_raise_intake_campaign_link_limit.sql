...
begin
  ...
  -- Serialize campaign creation per organization so concurrent requests
  -- cannot slip past the active-link limit.
  ...
  if (
    ...
    raise exception 'Limit reached: max 5 active multiple-visitor links per organization';
  end if;
