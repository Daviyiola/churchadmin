alter table public.form_fields
  add column layout_width text not null default 'full';

alter table public.form_fields
  add constraint form_fields_layout_width_check
  check (layout_width in ('full', 'half'));

comment on column public.form_fields.layout_width is
  'Responsive layout preference. Half-width fields pair on medium screens and stack on small screens.';
...
begin
  ...
  insert into public.form_fields (
    ...
    placeholder, is_required, options, layout_width, position
  )
  select
    p_form_id,
    v_form.org_id,
