  ...
  return <div className="grid gap-5 lg:grid-cols-[240px_minmax(0,1fr)]">
    ...
    <section className="overflow-hidden rounded-3xl border bg-white">
      ...
      <div className="min-h-[360px] space-y-5 px-5 py-6 sm:px-7">
        ...
        {step === 5 ? <><div className="text-sm font-semibold">How does your organization obtain permission to text people?</div><div className="grid gap-3 sm:grid-cols-2">{consentOptions.map(([value,label]) => <label key={value} className="flex items-center gap-3 rounded-2xl border p-4 text-sm"><input type="checkbox" checked={(draft.consent_methods ?? []).includes(value)} onChange={() => toggle("consent_methods", value)} />{label}</label>)}</div><div className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-700">Record each person?s verbal or written permission in the SMS permission tab. Choosing a collection method here does not enroll directory contacts. STOP requests and staff blocks always override permission.</div></> : null}
