...
export async function POST(req: Request) {
  ...
  if (callerMemErr) return NextResponse.json({ error: callerMemErr.message }, { status: 400 });
  ...
  if (targetMemErr) return NextResponse.json({ error: targetMemErr.message }, { status: 400 });
  if (!targetMembership) return NextResponse.json({ error: "User is not in this organization." }, { status: 404 });

  const targetRole = (targetMembership.role ?? null) as Role | null;

  // Only owners can remove admins/owners
  if ((targetRole === "admin" || targetRole === "owner") && !callerIsOwner) {
    return NextResponse.json(
      { error: "Only an owner can remove an admin/owner." },
      { status: 403 }
    );
  }
