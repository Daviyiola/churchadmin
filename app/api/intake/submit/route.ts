...
export async function POST(req: Request) {
  ...
  // Validate
  if (!firstName) return NextResponse.json<ErrorJson>({ error: "First name is required." }, { status: 400 });
  if (!email || !isValidEmail(email))
    return NextResponse.json<ErrorJson>({ error: "A valid email is required." }, { status: 400 });

  if (!lastName) return NextResponse.json<ErrorJson>({ error: "Last name is required." }, { status: 400 });
  if (!gender) return NextResponse.json<ErrorJson>({ error: "Gender is required." }, { status: 400 });
  if (!ageGroup) return NextResponse.json<ErrorJson>({ error: "Age group is required." }, { status: 400 });
  ...
  if (!maritalStatus)
    return NextResponse.json<ErrorJson>({ error: "Marital status is required." }, { status: 400 });

  if (!Number.isFinite(childrenCount) || childrenCount < 0) {
    return NextResponse.json<ErrorJson>(
      { error: "Children count is required and must be a valid non-negative number." },
      { status: 400 },
    );
  }
  ...
  if (consumeErr) return NextResponse.json<ErrorJson>({ error: consumeErr.message }, { status: 400 });
  ...
  if (upMemErr) return NextResponse.json<ErrorJson>({ error: upMemErr.message }, { status: 400 });
  if (upVdErr) return NextResponse.json<ErrorJson>({ error: upVdErr.message }, { status: 400 });
