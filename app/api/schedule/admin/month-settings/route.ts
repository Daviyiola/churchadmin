  ...
  try {
    ...
    const patch: Partial<
      Pick<
        ...
        "draft_open" | "is_public_visible" | "edits_open"
      >
    > = {};
    ...
    if (Object.prototype.hasOwnProperty.call(body, "draft_open")) {
      if (!isBool(body.draft_open)) {
        return NextResponse.json<ErrorJson>(
          { error: "draft_open must be boolean" },
          { status: 400 },
        );
      }
    ...
    if (Object.prototype.hasOwnProperty.call(body, "is_public_visible")) {
      if (!isBool(body.is_public_visible)) {
        return NextResponse.json<ErrorJson>(
          { error: "is_public_visible must be boolean" },
          { status: 400 },
        );
      }
    ...
    if (Object.prototype.hasOwnProperty.call(body, "edits_open")) {
      if (!isBool(body.edits_open)) {
        return NextResponse.json<ErrorJson>(
          { error: "edits_open must be boolean" },
          { status: 400 },
        );
      }
      patch.edits_open = body.edits_open;
    }
    ...
    const { data: updated, error: uErr } = await supabaseAdmin
      ...
      .select("id,month,draft_open,is_public_visible,edits_open")
      .single();
    ...
    return NextResponse.json({
      ...
      month: {
        ...
        edits_open: Boolean((updated as { edits_open?: unknown }).edits_open),
      },
    });
  } catch (e) {
