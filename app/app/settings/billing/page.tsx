...
import type { PublicPlan } from "@/lib/server/billing/catalog";

type Usage={used:number;limit:number|null};
type BillingData={role:string;plan:string;subscription:Record<string,unknown>|null;usage:Record<string,Usage|{used_cents:number;limit_cents:number|null}>};
const labels:Record<string,string>={members:"Active members",first_timers:"Active first-timers",management_seats:"Management seats",forms:"Custom forms",emails:"Emails this month",nikky:"Nikky allowance"};
...
export default function BillingPage(){
 ...
 return <div className="p-6"><div className="mx-auto max-w-6xl">
  ...
  {sub.cancel_at_period_end?<div className="mt-6 rounded-3xl border bg-amber-50 p-5">Your subscription will stop renewing at the end of its paid term. No records will be deleted.</div>:null}
  {data?.role==="owner"&&connected&&!sub.cancel_at_period_end?<button disabled={busy} onClick={()=>{if(confirm("Stop renewal at the end of the current paid term?"))action("/api/billing/cancel")}} className="mt-8 text-sm text-red-700 underline">Cancel renewal</button>:null}
 </div></div>;
}
